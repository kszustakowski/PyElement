...
class VesselClassifier(
    ...
    def __init__(self, feature_extractors, connectivity_extractors,
        ...
        :param feature_extractors: - list of methods for feature extraction
            from the image, each method returning
            (image_h, image_w, n_features) shape array
        :param connectivity_extractors: - list of methods for connectivity
            extraction from the mask, each method
            returning (image_h, image_w, n_features) shape array
        :param base_estimator: - estimator to use for fitting and prediction
        :param pca_dim: - number of target dimensions for
            PCA preprocessing (0 or less disables PCA)
        :param verbose: - print training info
        :param fit_mode: - 'static' for fitting using only the
                                target features & connectivity
                                features of the full mask
                         - 'growing' to simulate mask growing
                                during training feature generation
        :param prediction_proba_threshold: threshold for prediction probability
        :param connectivity_reduction_method: method for reducing the
                            connectivity mask connectivity features
                         - 'binary' - connectivity features are flag-like
                         - 'normalised' - connectivity features will be
                            normalised to 0-1 range based on the mask
                         - 'none' or None - no connectivity features reduction
        """
        ...
        self.prediction_proba_threshold = prediction_proba_threshold
        if connectivity_reduction_method is None:
            connectivity_reduction_method = 'none'
        self.connectivity_reduction_method = connectivity_reduction_method

    def fit(self, images_list, masks_list):
        """
        :param images_list: python list of images, each a single channel
            grayscale image in range 0-255 for uints or 0-1 for floats
        ...
        if self.fit_mode == "growing":
            X, y = self._fit_growing(init_masks, masks_list, image_features)
        elif self.fit_mode == "static":
            X, y = self._fit_static(init_masks, masks_list, image_features)
    ...
    def predict(self, images, proba_threshold=None):
        """
        :param images: python list of images, each a single
            channel grayscale image in range 0-255 for uints or 0-1 for floats
        :param proba_threshold: threshold for the prediction
            probability required to classify pixel as a vessel.
            If none, the probability specified at the classifier
            instantiation is used
        """

        if proba_threshold is None:
            proba_threshold = self.prediction_proba_threshold
        ...
        for image, image_features, mask in zip(images, image_features, masks):
            ...
            while not idempotent:
                mask_features = np.concatenate(
                    [extractor(prev_mask, self.connectivity_reduction_method)
                     for extractor in self.connectivity_extractors],
                    axis=-1
                )
                feature_vector = np.concatenate(
                    [image_features, mask_features], axis=-1)
                ...
                # Only predict for new entities
                to_predict = to_predict.flatten()
                feature_vector = feature_vector[to_predict > 0]

                new_mask = self.base_estimator.predict_proba(
                    feature_vector)[..., 1]

                new_mask_full = np.copy(prev_mask).astype(np.float64).flatten()
                new_mask_full[to_predict > 0] = new_mask
                new_mask = new_mask_full.reshape(mask.shape)
    ...
    @staticmethod
    def _mask_idempotent(mask, previous_mask, target_mask):
    ...
    def _fit_growing(self, init_masks, masks_list, image_features):
        X, y = [], []
        existing_feature_vectors = set()

        for idx, (v0, target_mask, vec_features) in \
                enumerate(zip(init_masks, masks_list, image_features)):
            if self.verbose:
            ...
            while True:
                ...
                if self._mask_idempotent(vi, vi_prev, target_mask):
                    break
                to_classify = vi > vi_prev
                vi_mask_features = np.concatenate(
                    [extractor(vi_prev, self.connectivity_reduction_method)
                        for extractor in self.connectivity_extractors],
                    axis=-1
                )
                feature_vector = np.concatenate(
                    [vec_features, vi_mask_features], axis=-1)
    ...
    def _fit_static(self, init_masks, masks_list, image_features):
        X, y = [], []
        for idx, (target_mask, vec_features) in \
                enumerate(zip(masks_list, image_features)):
            if self.verbose:
                print("Processing input image {}".format(idx))

            mask_features = np.concatenate(
                [extractor(target_mask, self.connectivity_reduction_method)
                    for extractor in self.connectivity_extractors],
                axis=-1
            )
            feature_vector = np.concatenate(
                [vec_features, mask_features], axis=-1)
