...
def ensure_flattened(filter_method):
    def filter_expanded_dim(image):
        if len(image.shape) > 2:
            image = np.squeeze(image)
        image = image.astype(np.float64)/255
        return filter_method(image)
...
def vesselness_filters(image):
    def vesselness_filter(scale):
        beta = 0.5
        c = 40
        hessian = skimage.feature.hessian_matrix(image, order='rc', sigma=scale)
...
def morphology(image):
    ...
    for conf in morphology_config:
        ...
        if conf['type'] == 'opening':
            source = cv2.erode(source, kernel=conf['B'], iterations=conf['erosions'])
            source = cv2.dilate(source, kernel=conf['B'], iterations=conf['dilations'])
        else:
            source = cv2.dilate(source, kernel=conf['B'], iterations=conf['dilations'])
            source = cv2.erode(source, kernel=conf['B'], iterations=conf['erosions'])
...
def light_sobel_filter(image, t=-10, d1=2, d2=5):
    # Rodrigues et al
    image = image*255
    b_image = cv2.copyMakeBorder(image, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=0)
